...
def bucketize_file(path, file, origin, distance_precision, time_precision):
    ...
    df = bucketize_dataframe(df, origin, distance_precision, time_precision)
...
def bucketize_dataframe(df, origin, distance_precision, time_precision):
...
def box(bucket, origin, distance_precision, time_precision):
    distances = [a*b for (a,b) in zip(bucket, [distance_precision, distance_precision, time_precision])]

    p1 = (origin[0] - distances[0] / 111111, distances[1] / 111111 / cos(radians(origin[0])) + origin[1], distances[2] + origin[2])
    
    distances = [(a+1)*b for (a,b) in zip(bucket, [distance_precision, distance_precision, time_precision])]
    p2 = (origin[0] - distances[0] / 111111, distances[1] / 111111 / cos(radians(origin[0])) + origin[1], distances[2] + origin[2])

    return [p1, p2]

def random(box):
    (p1, p2) = box
    return (uniform(p1[0], p2[0]), uniform(p1[1], p2[1]), int(uniform(p1[2], p2[2])))